...
class RPSGame
  ...
  def display_final_score
    return puts "#{display_final_winner} is the WINNER" if game_over?
  end

  def reset_score
    return human.score = 0 && computer.score = 0 if game_over?
  end
  ...
  def display_history
    ...
    puts human.history.display.to_s
    puts "- - - - - - - - - - - - - - - - - - - "
    puts "#{computer.name}'s History of moves is"
    puts computer.history.display.to_s
  end
